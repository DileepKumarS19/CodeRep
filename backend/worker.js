...
const worker = new Worker("code-execution-queue", async (job) => {
    ...
    try {
        ...
        const dockerCmd = `docker run --rm --network none --memory 512m --cpus 0.5 -v "${tempDir}":/app coderep-java sh -c "javac -cp /opt/junit.jar:/opt/assertj.jar *.java && java -cp /opt/junit.jar:/opt/assertj.jar:. org.junit.platform.console.ConsoleLauncher --select-class ${testClassName}"`;
